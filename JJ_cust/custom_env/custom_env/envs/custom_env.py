...
sourceDir = 'C:\\Users\\Patrick Wilk\\Documents\\RL\\JJ_first_Look\\SURPL-V2-JJ_first_look\\JJ_cust'
...
class ChargingStationEnv(Env):
    ...
    def reset(self):
        # self.time = 0
        self.timeStep = 0
        self.required = [1, 1, 0]    # need this in order to use demand charge 
        self.load = []
        self.TotalReward = 0 
        ...
        self.action_space = Box(low=0, high=self.largest, shape=(1,), dtype=float)
        self.observation_space = Box(low=0, high=self.largest, shape=(1,), dtype=float)
    ...
    def step(self, action):
        ...
        #dont work because will always choose 0- dont see the future max can add at the end ?
        ...
        print("Timestep: ", self.timeStep)
        print("-"*5)
        print("Deadline: ", self.chargingDeadline)
        print("-"*5)
        # print("Load: ", round(self.load, 2))
        ...
        print("Timestep: ", self.timeStep, file=fileOut)
        print("-"*5, file=fileOut)
        print("Deadline: ", self.chargingDeadline, file=fileOut)
        print("-"*5)
        # print("Load: ", round(self.load, 2), file=fileOut)
        ...
        #RETURNS NEGATIVE WHEN LOAD IS GREATER
        #CAN WE MAKE OBS THE TOTAL LOAD - REQURIED or (obs+= load)
        #DO WE WANT OBS TO BE GREATER OR LESS (DOES THIS EFFECT ANYTHING)
        observation = self.required[self.timeStep] - self.load[0]
        ...
        if self.timeStep <= self.chargingDeadline and sum([s for s in self.required])>= sum([i for i in self.load]):
        #if self.timestep < self.chargingDeadline:
            reward -= self.load[self.timeStep]+self.demandCharge
        if self.timeStep == self.chargingDeadline and sum([s for s in self.required])> sum([i for i in self.load]):
            reward -= (5*(sum([s for s in self.required]))-(sum([i for i in self.load]))) + self.demandCharge
        if self.timeStep > self.chargingDeadline:
            reward -= 10 + self.demandCharge
